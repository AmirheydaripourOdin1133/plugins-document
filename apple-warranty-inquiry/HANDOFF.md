...
> آخرین به‌روزرسانی سند: **2026-05** · نسخهٔ محصول: **1.1.5**
...
- جدول با کلاس‌های استاندارد `wp-list-table` و ستون‌های `hidden` / `column-primary` — در موبایل مثل **نوشته‌ها** فقط سریال + دکمه باز کردن جزئیات؛ بقیه با لمس ردیف.
...
1. وارد کردن کد امنیتی (تصویر) + شماره سریال (دکمه **×** سمت چپ فیلد برای پاک کردن سریع، مخصوصاً موبایل).
2. دکمه **بررسی شناسه**.
...
```php
// plugin.php — نسخه محصول (همه باید 1.1.5 باشند)
// * Version:           1.1.5   (هدر فایل پلاگین)
// * APPLE_WARRANTY_VERSION
// * readme.txt → Stable tag
APPLE_WARRANTY_VERSION      // '1.1.5' — cache-bust دارایی‌های CSS/JS
